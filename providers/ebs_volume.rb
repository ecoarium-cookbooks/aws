...
def create_volume(snapshot_id, size, availability_zone, timeout)
  ...
  unless new_resource.volume_type == "standard"
    options.merge!(:iops => new_resource.iops, :volume_type => new_resource.volume_type)
  end

  nv = ec2.create_volume(snapshot_id, size, availability_zone, options)
